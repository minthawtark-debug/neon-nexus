...
import { Users, Send, Bot, Power, Trash2, Activity } from "lucide-react";
...
const seedBots: Userbot[] = [
  { id: "1", username: "ghost_runner", phone: "+1 555 0142", active: true, forwards: 1240 },
  { id: "2", username: "synth_node", phone: "+44 207 1124", active: true, forwards: 880 },
  { id: "3", username: "void_proxy", phone: "+91 988 7710", active: false, forwards: 2100 },
  { id: "4", username: "neon_caster", phone: "+62 821 7700", active: true, forwards: 412 },
];

function AdminPage() {
  const [bots, setBots] = useState<Userbot[]>(seedBots);
  const totalUsers = 4821;
  const activeForwards = 37;
  const activeBots = bots.filter(b => b.active).length;

  const toggle = (id: string) => setBots(bs => bs.map(b => b.id === id ? { ...b, active: !b.active } : b));
  const remove = (id: string) => setBots(bs => bs.filter(b => b.id !== id));
  ...
  return (
    <Shell>
      ...
      <div className="mb-5 grid grid-cols-3 gap-2">
        <StatCard icon={Users} label="Total Users" value={totalUsers.toLocaleString()} color="cyan" />
        <StatCard icon={Send} label="Active Fwds" value={activeForwards} color="purple" />
        <StatCard icon={Bot} label="Userbots" value={activeBots} color="cyan" />
      </div>
          ...
          {bots.map((b, i) => (
            <div key={b.id} className="flex items-center gap-3 rounded-lg border border-border bg-[rgba(13,14,18,0.5)] p-3 transition hover:border-[rgba(0,240,255,0.4)] animate-float-up" style={{ animationDelay: `${i * 40}ms` }}>
              ...
              <div className="min-w-0 flex-1">
                <div className="truncate font-display text-sm font-semibold text-foreground">@{b.username}</div>
                <div className="font-mono text-[10px] text-muted-foreground">{b.phone} · {b.forwards} fwds</div>
              </div>
              <button onClick={() => toggle(b.id)} title={b.active ? "Deactivate" : "Activate"}
